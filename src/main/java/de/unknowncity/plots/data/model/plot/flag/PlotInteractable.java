...
public class PlotInteractable {
    ...
    private PlotInteractable(Material blockType, PlotFlagAccessModifier accessModifier) {
        this.blockType = blockType;
        this.accessModifier = accessModifier;
    }

    public boolean hasAccess(PlotMemberRole role) {
        if (accessModifier == PlotFlagAccessModifier.NOBODY) {
            return false;
        }
        if (accessModifier == PlotFlagAccessModifier.EVERYBODY) {
            return true;
        }
    ...
    public static List<PlotInteractable> defaults() {
        return allValidTypes().stream().map(material -> PlotInteractable.create(material, PlotFlagAccessModifier.TEMP_MEMBER)).toList();
    }

    public static PlotInteractable create(Material material, PlotFlagAccessModifier accessModifier) {
    ...
    public void accessModifier(PlotFlagAccessModifier accessModifier) {
        this.accessModifier = accessModifier;
    }
