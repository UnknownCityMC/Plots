...
public class PlotDenyCommand extends SubCommand {
    ...
    public void apply(CommandManager<CommandSender> commandManager) {
        commandManager.command(builder.literal("deny")
    ...
    private void handleDeny(@NonNull CommandContext<Player> context) {
        ...
        PlotUtil.getPlotIfPresent(sender, plugin).ifPresentOrElse(plot -> {
            ...
            if (plot.isOwner(target.getUniqueId())) {
                plugin.messenger().sendMessage(sender, NodePath.path("command", "plot", "deny", "owner"), plot.tagResolvers(sender, plugin.messenger()));
                return;
            }

            if (target.getUniqueId().equals(sender.getUniqueId())) {
                plugin.messenger().sendMessage(sender, NodePath.path("command", "plot", "deny", "self"), plot.tagResolvers(sender, plugin.messenger()));
                return;
            }
