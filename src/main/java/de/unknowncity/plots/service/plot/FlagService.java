...
public class FlagService extends Service<PlotsPlugin> {
    ...
    public <T> void setFlag(Plot plot, PlotFlag<T> flag, T value) {
        plot.setFlag(flag, value);
    }

    public void saveCurrentFlags(Plot plot) {
        CompletableFuture.runAsync(() -> saveCurrentFlags(queryConfiguration, plot));
    }
    ...
    public void setDefaults(Plot plot) {
        ...
        CompletableFuture.runAsync(() -> plotFlagDao.save(queryConfiguration, plot.id(), plot.flags()));
    }
}
